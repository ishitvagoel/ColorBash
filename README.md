...
overlay, macOS PTY matrix. Canonical status:
...
paints a reserved preview row (ADR 0015; `M-064` fixed). The completion overlay
is `complete` (M-065 reservation plus COLUMNS-1 clamp; type-to-filter GUI
`deferred`). Dim paint, type-to-filter overlays, and macOS matrix are
**G5 revisit**.
