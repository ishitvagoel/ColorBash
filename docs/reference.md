...
Full per-feature walkthroughs, the complete environment-variable list, and the
automated test commands. Start with [`README.md`](../README.md) if you
haven't installed MBX yet — this document assumes the comfort profile (or
`source bash/init.bash`) is already running and picks up where the README's
feature map leaves off.
