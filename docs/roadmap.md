...
- Active workstream: `SRCH-003` cwd prefix/fuzzy leftover recorded; overlay/ghost/highlighting blocked on unproven continuous decoration
...
and cwd-scoped empty-line/prefix/fuzzy search are complete (ADR 0009).
Age/status overlay, 100k interactive latency, repo/failed-command filters,
and signal/terminal state remain (`SRCH-003`). Ghost and highlighting stay
...
| `SRCH-003` | Metadata filters, 100k-row latency, signal, and terminal-state evidence | `validation` | cwd empty-line C-1–C-4 and cwd prefix/fuzzy C-5–C-8 in `bash/search.bash`, `crates/cli`, `crates/pty/tests/history_search.rs`, `tests/bash/modules.bash`; `docs/srch-003-cwd-filter-plan.md`; `docs/srch-003-cwd-prefix-plan.md`; overlay, 100k interactive, repo (`HIST-010`), failed-command, and signal leftovers remain |
...
1. `SRCH-001` and `SRCH-002` are `complete`. `SRCH-003` cwd empty-line and
   prefix/fuzzy filters are recorded; do not mark `SRCH-003` complete. Do not
   start overlay, ghost, or highlighting. `COMP-004` stays `discovery`. Do not
   mark `COMP-004` or `COMP-005` complete.
2. Next named leftovers are remaining `SRCH-003` work (overlay blocked; 100k
   deferred; repo/failed-command live on other PRs; signal/terminal-state) and
   the `HIST-010` / `GIT-003` pair. Do not duplicate those PRs on this branch.
