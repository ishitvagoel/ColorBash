...
- Active workstream: `COMP-002` wrap fallthrough (`COMP-002` in `validation`; `G4` in `discovery`; `G3` / `EDT-001` in `validation`)
...
| `COMP-002` | Prove file and one `-F` function's exact insertion parity | `validation` | `docs/comp-002-parity-plan.md`; P-1–P-4, F-1–F-4, L-1–L-4, N-1–N-2 landed; S-1–S-4 specified in `docs/comp-002-fallthrough-plan.md`; latency + `G4` decision remain |
...
1. `COMP-002` slow/stateful wrap fallthrough S-1–S-4 — follow
   `docs/comp-002-fallthrough-plan.md`. Adapter latency budget stays
   `deferred`. `G4` stays `discovery`. Do not mark `G3`, `EDT-001`,
