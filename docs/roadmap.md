...
- Active workstream: ADR 0011 + MBX2 QUERY wire for `GHST-001`; dim paint and continuous decoration remain blocked for highlighting / overlay
- Next decision gate: continuous-decoration leftover (blocks highlighting / overlay); wire ghost onto QUERY with generation stale rejection
- Editor-facing work: opt-in ghost suffix is unblocked (ADR 0010). Async QUERY wire is in `validation`; Bash stale rejection remains. Highlighting and GUI overlay stay blocked on after-every-key paint.
...
| 4 | Ghost suggestions | `validation` | ADR 0010 opt-in suffix; ADR 0011 + QUERY wire in validation; ghost stale rejection + dim leftovers remain |
...
1. `HIST-010` / `GIT-003` and ADR 0011 are on the async-unblock path. MBX2
   QUERY/RESULT/CANCEL wire is in `validation` (`docs/ghst-001-query-wire-plan.md`).
   Next leftover is Bash generation stale rejection + ghost install on QUERY.
   Ghost Strategy A sync CLI remains until that leftover. Dim paint remains
   blocked on continuous decoration. `GHST-004` latency is `deferred`. Do not
   mark `GHST-001` or `GHST-004` complete. Do not start highlighting or overlay.
...
3. `HRD-001` macOS remains Phase 9 / `G5`. Do not spend a slice on FND-001 SHA
   refresh or percentile benches unless a functional prompt-path defect is
   proven.
...
| 2026-08-20 | Landed MBX2 QUERY/RESULT/CANCEL wire (`docs/ghst-001-query-wire-plan.md`). `GHST-001` moves to `validation`. Bash ghost generation stale rejection remains. Do not mark `GHST-001` complete. |
