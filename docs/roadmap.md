...
- Current milestone: Phase 3 sidecar and Strategy A ghost are on main; `G0`–`G4` complete; ADR 0011 accepted; QUERY wire and ghost generation checks in `validation`; Strategy A search insert, cycling, and restore in `validation`
- Active workstream: overlapping delayed-RESULT PTY for `GHST-001`; remaining `SRCH-003` cwd/repo/failed/signal evidence
...
- Editor-facing work: opt-in ghost suffix is on main (ADR 0010). Async QUERY wire is in `validation`. Explicit history-search insert (`\C-xh`), cycling, and restore (`\C-xl`) are in `validation` (ADR 0009). Highlighting, dim paint, and GUI overlays are `deferred` from this MVP (G5 revisit)
...
| 8 | Enhanced Ctrl+R | `validation` | `\C-xh` insert, cycling, and `\C-xl` restore recorded (ADR 0009); overlay `deferred` |
...
Status: `validation` for explicit insert, bounded cycling, and restore
(ADR 0009). A type-to-filter overlay remains `deferred` (no decoration hook).
Default insert is `\C-xh`; restore is `\C-xl`.
...
| `SRCH-003` | Metadata filters, 100k-row latency, signal, and terminal-state evidence | `blocked` | remaining cwd/repo/failed interactive leftovers; CLI `search failed` / `search repo` / `search branch` are on main |
...
1. Overlapping delayed-RESULT PTY for `GHST-001` stale rejection (coprocess
   ...
   are met.
2. `SRCH-001` `\C-xh` insert/cycling and `SRCH-002` `\C-xl` restore are in
   `validation` (ADR 0009). Next leftover is `SRCH-003` interactive
   repo/failed/cwd/signal evidence. Overlay is `deferred`. Do not mark
   `SRCH-003` complete.
