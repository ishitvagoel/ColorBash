...
2. Install optional `bind -x` chords in emacs and vi-insert:
   - next: default `\C-x\C-n` (`MBX_COMP_CYCLE_NEXT_KEYSEQ`)
   - prev: default `\C-x\C-p` (`MBX_COMP_CYCLE_PREV_KEYSEQ`)
...
6. `COMP-004` stays `discovery`. Do not mark `COMP-004` or `COMP-005` complete.
...
- Percentile benches
- Marking `COMP-004` or `COMP-005` complete
...
explicitly narrowed in an ADR. GUI overlay remains unproven. Ghost,
highlighting, and enhanced Ctrl+R stay blocked on continuous decoration.
`COMP-005` stays blocked until overlay leftovers are addressed or the
deliverable is narrowed. `HRD-001` macOS matrix remains separate.
